function gameObject(){
    return {home : 
        {teamName : 'Brooklyn Nets',
         colors : ['Black', 'White'],
         players : 
   {'Alan Anderson' : {'number' : 0, 'shoe' : 16, 'points' : 16, 'rebounds' : 12, 'assists' : 12, 'steals' : 3,'blocks' : 1, 'slamDunks' : 1}, 
...
function teamColors(input){
    const colors = []    
    if (input === home.teamName){
    colors.push (game.home.colors)}   
    if (input === away.teamName){    
    ...
    score += (team[player].points) }        
        ...
        function winningTeam(){
            ...
            else {winningTeam = game.away.teamName}            
            return winningTeam            
            }